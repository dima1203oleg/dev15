...
const DesktopScene: React.FC<{ model: ThreatSceneModel }> = ({ model }) => {
  ...
  React.useEffect(() => {
    const contextPanel = stageRef.current?.querySelector<HTMLElement>('.desktop-context-panel');
    const intelligencePanel = stageRef.current?.querySelector<HTMLElement>('.desktop-intelligence-panel');
    contextPanel?.setAttribute('aria-label', 'Персональний контекст і регіональні шари');
    intelligencePanel?.setAttribute('aria-label', 'Оперативна інтелектуальна панель');
    intelligencePanel?.setAttribute('tabindex', '0');
  }, [model.dataMode, sceneMode]);

  return <div ref={stageRef} className={`device-scene device-scene--desktop desktop-command-stage desktop-command-stage--${sceneMode.toLowerCase()} ${isFullscreen ? 'desktop-command-stage--fullscreen' : ''}`}><h1 className="sr-only">SIREN UA Spatial Command Desktop</h1><h2 className="sr-only">Поточний режим просторової ситуаційної системи</h2>
...
const GlanceScene: React.FC<{ model: ThreatSceneModel; device: 'WATCH' | 'CAR' }> = ({ model, device }) => {
  ...
  return <div className={`device-scene device-scene--${device.toLowerCase()}`}><div className="flex items-center justify-between"><span className="text-[10px] font-mono tracking-[0.16em] text-slate-500">{isWatch ? 'GLANCE MODE' : 'SAFE DRIVE MODE'}</span>{isWatch ? <Watch className="h-4 w-4 text-cyan-300" /> : <Car className="h-4 w-4 text-cyan-300" />}</div><div className={`mt-4 rounded-2xl border p-4 ${isWatch ? 'border-rose-300/30 bg-rose-400/[0.08]' : 'border-cyan-300/25 bg-cyan-400/[0.06]'}`}><div className="flex items-center gap-2 text-xs font-bold text-white"><span className={`h-2 w-2 rounded-full ${isWatch ? 'bg-rose-300' : 'bg-cyan-300'}`} />{model.region.label}</div><div className="mt-2 text-lg font-black text-white">{model.risk.label}</div><div className="mt-1 text-xs text-slate-400">Орієнтовно: {model.dataMode === 'NOT_CONNECTED' ? '—' : leadEvent?.eta ?? '—'}</div></div><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" disabled title={isWatch ? 'Потрібна нативна інтеграція watch' : 'Потрібна нативна automotive voice integration'} className="min-h-11 rounded-xl border border-slate-700 bg-slate-950/70 px-3 text-xs font-bold text-slate-400 cursor-not-allowed">{isWatch ? 'Відкрити на телефоні' : 'Голосова інтеграція недоступна'}</button><button type="button" disabled title="Потрібні реальні shelter data та navigation provider" className="min-h-11 rounded-xl border border-slate-700 bg-slate-950/70 px-3 text-xs text-slate-400 cursor-not-allowed">{isWatch ? 'Деталі недоступні' : 'Укриття недоступні'}</button></div><div className="mt-4 text-[10px] leading-4 text-slate-500">{isWatch ? 'Один погляд · нативний handoff ще не підключений.' : 'Мінімум візуальних змін під час руху · voice provider не підключений.'}</div></div>;
};
...
export const DeviceExperienceLab: React.FC<{ dataMode: SpatialDataMode }> = ({ dataMode }) => {
  ...
  const renderScene = () => {
    switch (activeDevice) {
      ...
      case 'TABLET': return <TabletExperience model={model} />;
