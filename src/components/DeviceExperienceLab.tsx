...
const GlanceScene: React.FC<{ model: ThreatSceneModel; device: 'WATCH' | 'CAR' }> = ({ model, device }) => {
  ...
  return <div className={`device-scene device-scene--${device.toLowerCase()}`}><div className="flex items-center justify-between"><span className="text-[10px] font-mono tracking-[0.16em] text-slate-500">{isWatch ? 'GLANCE MODE' : 'SAFE DRIVE MODE'}</span>{isWatch ? <Watch className="h-4 w-4 text-cyan-300" /> : <Car className="h-4 w-4 text-cyan-300" />}</div><div className={`mt-4 rounded-2xl border p-4 ${isWatch ? 'border-rose-300/30 bg-rose-400/[0.08]' : 'border-cyan-300/25 bg-cyan-400/[0.06]'}`}><div className="flex items-center gap-2 text-xs font-bold text-white"><span className={`h-2 w-2 rounded-full ${isWatch ? 'bg-rose-300' : 'bg-cyan-300'}`} />{model.region.label}</div><div className="mt-2 text-lg font-black text-white">{model.risk.label}</div><div className="mt-1 text-xs text-slate-400">Орієнтовно: {model.dataMode === 'NOT_CONNECTED' ? '—' : leadEvent?.eta ?? '—'}</div></div><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" className="min-h-11 rounded-xl border border-cyan-300/30 bg-cyan-400/10 px-3 text-xs font-bold text-cyan-100">{isWatch ? 'Відкрити карту' : 'Голосове повідомлення'}</button><button type="button" className="min-h-11 rounded-xl border border-slate-700 bg-slate-950/70 px-3 text-xs text-slate-300">{isWatch ? 'Закрити' : 'Укриття поруч'}</button></div><div className="mt-4 text-[10px] leading-4 text-slate-500">{isWatch ? 'Один погляд · одна дія.' : 'Мінімум візуальних змін під час руху.'}</div></div>;
};
