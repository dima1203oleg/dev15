...
- **Not proven by this repository**: connected production identity/RBAC deployment, provider-specific payment normalization/qualification, real payout-provider dispatch/reconciliation deployment, KYC/AML, tax handling, authoritative geography and real-time provider stability. PostgreSQL migrations/probe, authenticated partner read projections, the signed subscription transition webhook boundary, the signed payment qualification webhook boundary and the signed durable payout settlement webhook boundary are implemented and locally verified.
