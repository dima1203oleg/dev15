...
`POST /api/webhooks/:provider/payout` is the signed payout settlement gateway. The caller must send the exact normalized JSON envelope `{ payoutId, providerPayoutId, status: "PAID" | "FAILED", occurredAt }` plus `x-siren-event-id`, `x-siren-timestamp` and an HMAC-SHA256 `x-siren-signature` over `timestamp.rawBody` using `PAYOUT_WEBHOOK_SECRET`. The gateway verifies the signature before parsing the payload, stores the raw event, applies provider-event idempotency and settles the durable ledger exactly once. Reusing an event ID with different raw bytes returns `409 WEBHOOK_IDEMPOTENCY_CONFLICT`; it never overwrites the original event. It returns `503 PAYOUT_WEBHOOK_NOT_CONNECTED` until the database and webhook secret are configured.

`POST /api/webhooks/:provider/subscription` is the signed billing state gateway for `WEB`, `APPLE` and `GOOGLE`. Its normalized envelope is `{ subscriptionId, event, occurredAt, providerSubscriptionId?, billingConsentAt?, currentPeriodStart?, currentPeriodEnd? }`. It verifies the exact raw body with `PAYMENT_WEBHOOK_SECRET`, persists provider metadata and applies the subscription state machine exactly once. A duplicate event with identical raw bytes is acknowledged as `DUPLICATE`; the same event ID with different raw bytes is rejected. This route changes subscription state only; it does not infer a qualified payment or commission, which requires the separate verified payment pipeline.
