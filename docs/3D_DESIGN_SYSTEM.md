...
`src/components/DeviceExperienceLab.tsx` містить окремі сцени для кожного класу пристрою; `src/data/spatialModel.ts` є shared data contract. `DEMO_SPATIAL_MODEL` — тільки візуальна fixture і завжди має маркування `DEMO DATA`.
...
- WebGL не є обов’язковим: CSS 3D дає доступний fallback для core prototype.
