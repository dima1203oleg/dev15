...
When a provider is connected, the policy must be evaluated through an idempotent scheduled worker, which creates a durable payout request and hands the lock to `PayoutDispatchRepository`. Provider dispatch, signed webhook settlement and reconciliation remain the only paths allowed to move a request from `PAYOUT_HELD` to `PAID` or restore it to `AVAILABLE`.
