...
The web server performs a bounded `SELECT 1` probe when `DATABASE_URL` is present. A configured URL with an unreachable database reports `ERROR` in `/api/ready` and cannot be treated as a healthy dependency. The current partner HTTP routes still remain disabled until identity/authentication and the durable repository wiring are supplied; the probe is not a substitute for that wiring.
