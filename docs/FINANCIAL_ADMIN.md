...
The scheduled-to-active step is a worker boundary: `FinancialRuleRepository.activateDue(at)` atomically claims due schedules with `FOR UPDATE SKIP LOCKED`, marks them `ACTIVE`, and writes an immutable audit record. `getActive(ruleType, at)` resolves the latest effective version without reopening or rewriting historical financial records.
