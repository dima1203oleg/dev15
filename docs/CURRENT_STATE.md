...
- API: Express in-memory demo endpoints for threat data and partner projections; network responses are bounded and strip internal user/referrer IDs, payout history masks destinations. Financial domain rules live in a pure typed module. Production financial routes return `NOT_CONNECTED` until durable integrations exist.
...
- CI: GitHub Actions verification workflow covers type, domain, boundary, PostgreSQL migrations/repositories, browser/accessibility, build, performance and dependency checks; staging deployment, provider-specific billing normalization, observability and external payout provider remain missing/unverified.
- Partner Platform 4.0: exact minor-unit/QCB/cap/rank/ledger/FX/payout-gate primitives, durable PostgreSQL qualification/payment/payout boundaries and acceptance suite exist; provider-specific billing, KYC/AML, FX and payout adapters remain unconnected.
